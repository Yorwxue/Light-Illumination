...
#define NumofParticle 16382       //取NumofParticle組解作為PSO的初值(粒子數) ，每個particle大小為200
#define NumofRedistribute 8192
...
float Host_I[NumofInstruction][2];   //(耗能,亮度)
float Host_Td[NumofTd];
float Host_KParameter[NumofTd][NumofLight + OtherLight];
...
{
private:
	int PBest[NumofLight];  //(第一盞燈指令,第二盞燈指令,第三盞燈指令,個體最低耗能)
	float PBestFitness;
...
public:
	...
	********/
	__device__  void DeviceFunction(int* GBestPointer, float NewGBestFitness, float C0, float C1, float C2, float* I, float* Td, float* KParameter, int ParticleNoinGroup)
	{
		//SetGBest
		GBestFitness = NewGBestFitness;
...
__global__ void PSOKernel(Particle* Group, int* GBest, float* GBestFitness, int GroupCircle, float* I, float* Td, float* KParameter)
{
	...
	if (ParticleNoinGroup < NumofParticle && CoordinatesX < (ThreadX*BlockX) && CoordinatesY < (ThreadY*BlockY))
		...
		/*****/
		(Group + ParticleNoinGroup)->DeviceFunction(GBest, *(GBestFitness), C0, C1, C2, I, Td, KParameter, ParticleNoinGroup);
		/*****/
	...
	else
	{
		...
		for (int i = 0; i<NumofInstruction; i++)
		{
			for (int j = 0; j<2; j++)
			{
				...
				//I[i][j]=Host_I[i][j];  //將值送到共用(shared)的memory
				//cout<<Host_I[i][j]<<",";
			}
			//cout<<endl;
		}
	...
	else
	{
		...
		for (int i = 0; i<NumofTd; i++)
		{
			for (int j = 0; j<(NumofLight + OtherLight); j++)
			{
				...
				//sscanf(FileInput,"%f",&KParameter[i][j]);
				//KParameter[i][j]=Host_KParameter[i][j];
...
float PSO()
{
	...
	int TotalGBest[NumofLight];
	float TotalGBestFitness = ImpossibleResume;

	int GBest[NumofGroup][NumofLight];
	float GBestFitness[NumofGroup];
	...
	Particle GroupInfo[NumofGroup][NumofParticle];
